...
deploy_cert() {
  ...
  case "${DOMAIN}" in
    "erp.primtec.at")
      rsync --password-file /etc/rsync.password "${CERTFILE}" riprod02@odoo01.primtec.at::odoo/nginx-selfsigned.crt
      rsync --password-file /etc/rsync.password "${KEYFILE}" riprod02@odoo01.primtec.at::odoo/nginx-selfsigned.key
      ;;
  esac
